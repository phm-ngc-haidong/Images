...
// Tạo hình tinh trung tâm

// Hàm tạo texture cho hàn tinh
function createPlanetTexture(size = 512) {
    ...
    const spotColors = ['#f8bbd0', '#f8bbdf', '#f48fb1', '#f49fb1', '#f06292', '#f06282', '#ffffff', '#e1aaff', '#a259f7'];
    for (let i = 0; i < 40; i++) {
...
// Shader cho hiệu ứng bão trên bề mặt hanh tinh
const stormShader = {
    uniforms: {
        time: { value: 0.0 },
...
// Tạo các vòng chứa quanh hành tinh
const ringTexts = [
   '-----',   
]; 
...
function createTextRings() {
    ...
    for (let i = 0; i < numRings; i++) {
        ...
        // Logic phân tích và tự động điều chỉnh kích thước font (giữ nguyên font-family)
...
/*
 ...
 Phiên bản hoàn chỉnh: Con trỏ màu trắng đồng nhất và được đặt ở vị trí cố định
...
function createHintIcon() {
    ...
    // 1.Tạo hình dáng con trỏ (giữ nguyên)
    ...
    // 3.Tạo vòng trắng bao quanh (giữ nguyên)
...
function animateHintIcon(time) {
    ...
    if (!introStarted) {
        ...
        // Di chuyển icon tới lui theo hướng nhìn
        ...
        ring.material.opacity = 0.5 + Math.sin(time * tapFrequency) * 0.2;
         // Xử lí văn bản gợi ý (thêm hiệu ứng mới)
        if (hintText) {
...
function createHintText() {
    ...
    context.lineWidth = 2;
    context.strokeStyle = 'transparent'; // Không có màu nền
    ...
    context.fillStyle = 'transparent'; // Không có màu nền
...
// Các hàm xử lí sự kiện và khỏi động
createShootingStar();
...
// Các thiết lập cho giao diện và điện thoại
